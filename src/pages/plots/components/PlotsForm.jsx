...
export default class PlotsForm extends Component {
  ...
  searchAccessionIds = (accession) => {
    ...
    for (let i = 0; i < rowNames.length; i++) {
      if (rowNames[i].includes(accession)) accessionIdsView.push(rowNames[i]);
  ...
  onPlotGeneButtonClick = (event) => {
    ...
    plotStore.addPlot(this.state.accessionIds, {
    ...
    return (
      ...
      >
          ...
          <AppSelect
            ...
            options={[
              { label: 'group', value: 'group' },
              {
                label: 'gene',
                value: 'gene',
                disabled: this.state.accessionIds.length === 1,
              },
            ]}
          ...
          return (
            ...
            <div key={index} className="flex items-center mt-4">
              <AppDatalist
                ...
                onChange={(accession) =>
                  this.onAccessionDataListChange(accession, index)
                }
                onSelect={(accession) =>
                  this.onAccessionDataListChange(accession, index)
                }
              ...
              <AppIcon
                className={`ml-4 w-12 h-12 cursor-pointer
                     ${isLast ? 'text-green-700 mb-8' : 'text-pink-700'}`}
                file="hero-icons"
                id={isLast ? 'plus' : 'minus'}
                onClick={() =>
                  this.onAccessionDatalistIconClick(
                    isLast ? 'add' : 'remove',
                    index
                  )
                }
        ...
        <div className="mt-8 flex justify-start w-full">
          <AppButton
            className={`px-5 py-2 shadow-outer text-white
              ${
          ...
          >
            ...
            {this.state.loading ? 'Plotting' : 'Plot Gene'}
          </AppButton>
