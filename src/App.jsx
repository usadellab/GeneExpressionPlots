...
export default class App extends React.Component {
  async componentDidMount() {
    ...
    if (settings.preloaded.data) {
      const dataResponse = await fetchResource(settings.preloaded.data, {
        type: 'text',
      });
      if (dataResponse)
    ...
    if (settings.preloaded.info) {
      const infoResponse = await fetchResource(settings.preloaded.info, {
        type: 'text',
      });
      if (infoResponse)
    ...
    if (settings.preloaded.image) {
      const image = await fetchResource(settings.preloaded.image, {
        type: 'url',
      });
